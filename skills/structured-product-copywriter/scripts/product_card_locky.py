...
def main():
    ...
    with sync_playwright() as pw:
        ...
        # 14: 入场时间, 15: 入场点位
        # 降落伞月份固定填1=最后1个月用降落伞值，其他月份逐月降敲(101%→100.5%→...→85%)。
        # 填36会导致所有月份都变成降落伞值(表单bug)。实测降落伞月份=1→月3-35逐月降敲+月36=降落伞。
        values = {
            0: args.title,
            1: args.term,
            2: args.lock,
            ...
            4: args.ko,
            5: args.step_down,
            6: args.parachute,         # 降落伞值(如65)
            7: 1,                       # 固定1=最后1个月用降落伞，不用args.parachute_months
            8: args.rate1_start,
            9: args.rate1_end,
            10: args.rate1,
            11: args.rate2_start,

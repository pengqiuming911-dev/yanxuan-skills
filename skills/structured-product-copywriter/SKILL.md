...
- 第 11 节展示 `账户:`、`账号:`、`银行:` 三个字段，**在打款备注下一行附上核验链接或截图**。核验方式查找：根据第 10 节产品公示里的**托管人名称**，在飞书文档 `https://kcngap16uccc.feishu.cn/docx/TzWXdKAaeol7kTxs3BNcowqunJh` 里的"Q:募集户我要如何核对？"部分查找该托管人的核验链接/截图（每个托管人的募集户核验链接或截图都在这个飞书文档里标注了），提取后放到本次销售物料的募集户核对文案末尾。涉及募集账户相关的问题都按这个流程。找不到该托管人的核验信息→写 `[托管户核验方式待补]`。

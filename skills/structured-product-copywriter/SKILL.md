...
- 第 11 节展示 `账户:`、`账号:`、`银行:` 三个字段，**在打款备注下一行直接附上核验链接和截图**（不是写"见飞书FAQ"这种话——必须把链接和图片提取出来放到物料文档里）。核验方式查找流程：
  1. 从第 10 节产品公示里拿到**托管人名称**（如"华泰证券"）。
  2. 用浏览器（xvfb-run + Playwright）打开飞书文档：`https://kcngap16uccc.feishu.cn/docx/TzWXdKAaeol7kTxs3BNcowqunJh`
  3. 文档结构：**每个 H4 标题就是一个托管人名称**。找到与托管人名称匹配的 H4 标题。
  4. 该 H4 标题下有一句"**募集户核对地址：**"，这句话**下面紧跟着的链接和图片**就是该托管人的募集户核对地址。
  5. **直接把这个链接 URL 和图片提取出来**（截图保存到 assets/custody-<托管人>.png），放到 manifest 第11节 body 里（`账户:... 账号:... 银行:... 募集户核对地址：<链接URL>`）+ image（截图路径）。
  6. 涉及募集账户相关的问题都按这个流程。找不到该托管人的 H4 或"募集户核对地址："→写 `[托管户核验方式待补]`。
